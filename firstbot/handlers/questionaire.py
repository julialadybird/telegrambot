...
async def form_name(message: Message, state: FSMContext):
    ...
    await message.answer("Enter your age", reply_markup=rmk)
...
async def form_sex(message:Message, state: FSMContext):
    ...
    await message.answer("About yourself", reply_markup=rmk)

@router.message(Form.sex)
async def incorrect_form_sex(message:Message, state: FSMContext):
    await message.answer("Enter the button")


@router.message(Form.about)
async def form_about(message:Message, state: FSMContext):
    if len(message.text) < 5:
        await message.answer("Enter something better")
    else:
...
async def form_photo(message:Message, state: FSMContext, bot: Bot):
    ...
    await bot.send_photo(message.from_user.id, photo=photo_file_id)
    await message.answer("\n".join(formatted_text))
